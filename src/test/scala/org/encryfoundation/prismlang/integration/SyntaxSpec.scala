...
class SyntaxSpec extends PropSpec with Matchers with Parser with TestCompiler {
  ...
  property(testName = "Single Static Assignment") {
    val correctOrder =
      """
                {
                  let a = 10
  ...
  property(testName = "stack overflow") {
    ...
    //FIXME Out of memory with 100M array size
    val hugeArray = getArrayString(List.range(1, 10000000))
  ...
  def compiled(prismScript: String) : Try[Ast.Expr] = compileExpr(parse(prismScript).get.head)
