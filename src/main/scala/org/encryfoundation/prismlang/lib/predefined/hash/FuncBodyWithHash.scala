...
trait BuiltInHashHolder extends BuiltInFunctionHolder{

  def asFunc: PFunctionPredef = PFunctionPredef(args, body)

  val args = IndexedSeq("input" -> Types.PCollection.ofByte)

  protected def bodyValue(hash: Array[Byte] => Array[Byte]): Seq[(String, PValue)] => Either[PFunctionPredef.PredefFunctionExecFailure.type, Any] = (args: Seq[(String, PValue)]) => {
